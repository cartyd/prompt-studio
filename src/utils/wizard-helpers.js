/**
 * Wizard utilities for EJS templates
 * Eliminates complex conditional logic in wizard templates
 */
...
const WizardUtils = {
   ...
   */
  renderProgressText(current, total) {
    return `
   ...
   */
  renderNavigation(stepNum, totalSteps) {
   ...
   */
  renderScript(question, totalSteps, stepNum) {
    return `
   ...
   */
  getRecommendationHeading(confidence) {
    if (confidence >= 70) {
      return {
   ...
   */
  renderFrameworkCard(recommendation) {
    return `
   ...
   */
  renderExplanation(recommendation) {
    return `
   ...
   */
  renderActionButtons(recommendation) {
   ...
   */
  renderConfidenceTip(confidence) {
    if (confidence < 50) {
      return `
